      ...
      <div className={`relative ${containerClassName}`}>
        <label
          ...
          className={`absolute left-0 transition-all duration-200 ease-in-out pointer-events-none ${
            isFloating ? '-top-5 text-xs text-gray-400' : 'top-2 text-base text-gray-500'
          }`}
        >
          {label}
        </label>
        ...
        <div className="relative flex items-center">
          <input
            ...
            } text-white py-2 transition-colors duration-200 outline-none ${icon ? 'pr-8' : ''}`}
            {...props}
          />
          {icon && (
            <div
            ...
            >
              {React.cloneElement(icon, {
                className:
                  'w-5 h-5 text-gray-500 hover:text-gray-300 transition-colors',
              })}
            </div>
          )}
        </div>
